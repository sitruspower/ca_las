...
for i=1:dim
    for j=1:dim
        for k=1:dim
                ...
                struct.fs(i,j,k) = 0.5;
                struct.alpha(i,j,k) = rand(1)*pi/2;
                struct.beta(i,j,k) = rand(1)*pi/2;
                struct.gamma(i,j,k)= rand(1)*pi/2;              
...
if OneGrain==1
    active = [1,1,1];
    struct.alpha(active(1,1),active(1,2),active(1,3)) = angle_alpha;
    struct.beta(active(1,1),active(1,2),active(1,3)) = angle_beta;
    struct.gamma(active(1,1),active(1,2),active(1,3)) = angle_gamma;
else 
    active = [2,2,7;
              3,3,6;
              3,3,5;
...
struct = func_growth_in_sl_parallel3D(active, struct, timelimit, timestep, dx, xmin,ymin,zmin, 1);
